...
REF_NAME="${1:-"main"}"
STATUS_URL="/repos/kyma-project/keda-manager/commits/${REF_NAME}/status"

function verify_github_jobs_status () {
	local number=1
	while [[ $number -le 100 ]] ; do
		echo ">--> checking keda job status #$number"
		local STATUS=`gh api -H "Accept: application/vnd.github+json" -H "X-GitHub-Api-Version: 2022-11-28" ${STATUS_URL} | jq -r .state `
